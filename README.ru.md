...
- Параллелизм Flink по умолчанию выше числа партиций молча удерживает watermark на −∞: джоба выглядит здоровой, потребляет данные, но окна не срабатывают ни разу. Для топика из 3 партиций нужно `parallelism.default = 1`.
- Зелёный прогон Airflow не доказывает, что работа выполнена: DAG с неверным отступом распарсился с **нулём задач**, и каждый запуск при этом горел зелёным. Всегда проверяйте сам граф задач.
- Great Expectations 1.2: `value_set` должен быть списком, а не множеством Python — множества сериализуются в недетерминированном порядке и ломают проверку актуальности ресурсов.
- Неотсортированный поток событий делает оконную обработку по event time бессмысленной: перед воспроизведением нужно либо сортировать, либо осознанно проектировать под опоздания через watermarks.
- URI DuckDB в Superset требует четырёх слэшей для абсолютного пути: `duckdb:////path/to/db.duckdb`.
